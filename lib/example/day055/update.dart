...
class _UpdateSate extends State<Update> { // 클래스명 앞에 _ 언더바는 dart에서 private 키워드
  ...
  void todoFindById( int id ) async {
    try{
      final response = await dio.get("http://221.168.154.198:8080/api/todo/view?id=$id");
  ...
  void todoUpdate(  ) async{
    try{
      ...
      final response =  await dio.put("http://221.168.154.198:8080/api/todo" , data : sendData );
      final data = response.data;
